...
def setup_sidebar():
    ...
    with st.sidebar:
        ...
        if not all([ st.session_state.openrouter_api_key, st.session_state.qdrant_url]):
...
def web_search(state):
    ...
    try:
        ...
        print("TAVILY KEY:", st.session_state.tavily_api_key[:10])
...
def retrieve(state):
    ...
    st.info(f"Retrieved {len(documents)} documents")
    return {"keys": {"documents": documents, "question": question}}
...
documents = value['keys'].get('documents', [])
